...
void camera_open(int id)
{
    ...
    CAMERA_OPS(id)->camera_open(&camera_devs[id]);
}

void camera_close(int id)
{
    if (!CAMERA_OPS(id)->camera_close)
        return;
...
void gimbal_open(int id)
{
    ...
    CAMERA_OPS(id)->gimbal_open(&camera_devs[id]);
}

void gimbal_close(int id)
{
    if (!CAMERA_OPS(id)->gimbal_close)
        return;
...
{
    if (!CAMERA_OPS(id)->gimbal_centering)
        return;
...
{
    if (!CAMERA_OPS(id)->gimbal_rotate)
        return;
